...
int main(void)
{
  ...
  uint8_t masterTx[size] = {REG_ADDR, 0x00};

  uint8_t cmd[1] = {0x00};

  uint8_t masterRx[6];

 if( HAL_I2C_Master_Transmit_DMA(&hi2c1, ADDR, masterTx, size) == HAL_OK)
 {
    //if the transmition was successful

   //could also use these flags: HAL_ERROR  :  HAL_BUSY  :  HAL_TIMEOUT

 }

  while (1)
  {
    HAL_Delay(50 + masterRx[0]);
    HAL_I2C_Master_Transmit_DMA(&hi2c1, ADDR, cmd, 1);

    HAL_Delay(10);

    HAL_I2C_Master_Receive_DMA(&hi2c1, ADDR, masterRx, 6);

    //this does some fancy bit shifting magic to get the data correct
    for(int i = 0; i < sizeof(masterRx); i++)
    {
      masterRx[i] = (masterRx[i] ^ 0x17) + 0x17;
    }
